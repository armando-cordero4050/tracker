...
const collectEntries = () => {
  const fileList = glob.sync(`wwwroot/vendor/!(${conf.exclude.join('|')})/**/!(_)*.@(js|es6)`) || [];
  const fileListFiltered = fileList.filter(str => !str.includes('formvalidation'));
  return fileListFiltered.filter(p => !/\.dist\.(?:js|css)$/.test(p)).reduce((entries, file) => {
    const filePath = file.replace(/\\/g, '/');
    return { ...entries, [filePath.replace(/^wwwroot\/vendor\/|\.(?:js|es6)$/g, '')]: `./${filePath}` };
  }, {});
};
